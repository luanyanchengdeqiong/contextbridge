...
- **多 IDE 自动采集**:自动探测 Claude Code / ZCode / Codex 的本地会话,无需手动粘贴
...
```bash
# 1. 在 Claude Code 侧,导出当前会话(自动探测本地 session)
...
**远程 / 网络测试**用 Streamable HTTP(无状态核心,符合 2026-07-28 规范):

```bash
cb serve --http -p 8000          # 然后在 host 配置里用 "url": "http://127.0.0.1:8000/mcp"
```

详见 [docs/install.md](docs/install.md#21-streamable-http-模式远程--网络测试)。
...
contextbridge/
├── src/contextbridge/
│   ├── server.py            # MCPServer (2.0) + 4 个 cb_* 工具
│   ├── cli.py               # Typer CLI
...
│   ├── config.py            # 路径管理(CONTEXTBRIDGE_HOME / 默认 ~/.contextbridge)
│   └── adapters/            # IDE 适配器
│       ├── base.py          #   SourceAdapter Protocol
│       ├── claude_code.py   #   Claude Code (~/.claude/projects/)
│       ├── zcode.py         #   ZCode (~/.zcode/cli/agents/)
│       ├── codex.py         #   Codex (~/.codex/sessions/)
│       └── generic.py       #   Generic(外部入参兜底)
├── tests/                   # 38 条测试
├── docs/
│   ├── install.md
│   └── architecture.md
...
覆盖:存储 CRUD、前缀查找、孤儿行兜底、FTS 检索、CLI 各命令、MCP server 工具、各 IDE 适配器(Claude Code / ZCode / Codex / Generic)、渲染格式、git diff、裁剪。
